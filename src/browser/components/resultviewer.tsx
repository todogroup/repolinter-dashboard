...
export default class ResultViewer extends React.Component<Props, State> {
  ...
  public getFaIcon(status: boolean) {
    if (status) {
      return (
        <i className="fa fa-thumbs-o-up green" aria-hidden="true">
          <strong>Passed</strong>
        </i>
      );
    } else {
      return (
        <i className="fa fa-exclamation-triangle yellow" aria-hidden="true">
  ...
  public async color() {
    ...
    if (
      this.state.spinnerColor === '' ||
      this.state.spinnerColor === '#FFD036'
    ) {
      this.setState({
        spinnerColor: '#97CD34',
      });
  ...
  public renderResult() {
    if (this.props.result.length !== 0) {
      ...
      return (
        <div className="sweet-loading center-div">
          <PropagateLoader
